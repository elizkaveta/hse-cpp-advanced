...
private:
    ...
    void MinusBlock() {
        if (block_ && block_->cnt_ == 0 && block_ != nullptr) {
            delete block_;
            ptr_ = nullptr;
        }
    }
...
SharedPtr<T> MakeShared(Args&&... args) {
    ControlBlockHolder<T>* new_block = new ControlBlockHolder<T>(std::forward<Args>(args)...);
    return SharedPtr<T>(new_block->GetUk(), new_block);
}
